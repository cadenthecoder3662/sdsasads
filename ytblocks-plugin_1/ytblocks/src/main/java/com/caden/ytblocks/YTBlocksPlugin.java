...
/**
 * YTBlocks - carves blocks out of a defined play area as a YouTube video's
 * views/likes and the channel's subscriber count increase.
 *
 * How it works:
 *  - On enable, sets the world border to match the configured play area
 *    (so no manual /worldborder commands are needed).
 *  - Every poll-interval-seconds, asynchronously calls the YouTube Data API
 *    for the video's viewCount/likeCount and the channel's subscriberCount.
 *  - Compares against the last known values (persisted in data.yml so
 *    restarts don't cause a jump).
 *  - Converts that growth into a number of blocks "owed", then walks a
 *    persistent scan cursor through the play area (top of each column down
 *    to the bottom) turning solid blocks to air until the debt is paid off,
 *    capped per check so a big stat jump can't cause a lag spike.
 */
public class YTBlocksPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        // 100 ticks = 5s initial delay, then every poll-interval-seconds
    ...
    /** Runs asynchronously on the scheduler. Does the HTTP calls off the main thread. */
    private void pollAndApply() {
        try {
            ...
            long[] videoStats = fetchVideoStats(videoId); // [views, likes]
            ...
            if (lastViews < 0) {
                // First run: establish a baseline without removing anything,
                // otherwise the video's entire lifetime view count would
                // instantly carve out the whole play area.
                lastViews = views;
                lastLikes = likes;
                lastSubs = subs;
                saveData();
            ...
            // Block edits must happen on the main server thread.
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length > 0 && args[0].equalsIgnoreCase("reload")) {
            ...
            loadSettings();
            applyWorldBorder();
            sender.sendMessage("§a[YTBlocks] Config reloaded.");
            return true;
        }

        sender.sendMessage("§b[YTBlocks] " + blocksRemoved + " blocks removed. Scan progress: "
                + scanCursor + "/" + totalCells + ". Blocks owed: " + pendingCredit);
        sender.sendMessage("§b[YTBlocks] Last known - views: " + lastViews
